...
export const Administrate = ()=> {
    ...
    const addExe = ()=> {
        ...
        Axios.post("/api/executive/addExe", 
    ...
    const showExes = ()=>{
        Axios.get("/api/executive/exeDetails").then((response)=> {
    ...
    const updexe = (id)=>{
        if(exeName.length > 0 && exeMobNo.length == 10){
        Axios.put("/api/executive/updateExe", 
    ...
    const delexe = (id)=> {
        Axios.delete(`/api/executive/deleteExe/${id}`).then((response)=> {
    ...
    const updVeh = (id)=>{
        if(vehModel.length > 0 && vehType.length == 1){
            Axios.put("/api/vehicle/updateVeh", 
    ...
    const delVeh = (id)=> {
        Axios.delete(`/api/vehicle/deleteVeh/${id}`).then((response)=> {
            alert('Delete successfully');
        });
    };
