...
export const Pending = ()=> {
    ...
    //required methods
    useEffect(()=> {
        Axios.get("/api/testdrive/viewPending").then((response)=> {
    ...
    /*const showPending = ()=>{
        Axios.get("/viewPending" ).then((response)=> {
            if(response.data.length > 0){
                setPending(response.data);
                console.log(response);
            }
    ...
    const Confirm = (id)=>{
        if(exeId.length > 0){
            Axios.put("/api/testdrive/confirm", 
            { tid : id,
            eid : exeId
            }).then((response)=> {
    ...
    const availExe = ()=>{
        if(td_date.length > 0 && td_slot.length > 0){    
            Axios.post("/api/executive/availExe",{
                date : td_date,
                slot : td_slot
            }).then((response)=> {
    ...
    const vehAvailCheck = ()=>{
        if(td_date.length > 0 && td_slot.length > 0 && vehId != 0){    
            Axios.post("/api/vehicle/vehAvailable",{
                id : vehId,
                date : td_date,
                slot : td_slot
            }).then((response)=> {
